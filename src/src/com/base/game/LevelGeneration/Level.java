...
//	}
	...
	public void initRooms(int shipSize){initRooms(shipSize, 0);}
	public void initRooms(int shipSize, int techLevel){initRooms(shipSize, techLevel, 0);}
	public void initRooms(int shipSize, int techLevel, int militaryLevel){initRooms(shipSize, techLevel, 0, 0);}
	public void initRooms(int shipSize, int techLevel, int militaryLevel, int additional)
	{
		...
		while(i < rooms.length){
			...
			if(rooms.length - i == 1){
				...
				if(axisPlace(hw, yheight, yh, i, registry)) i++;
				else done--;}
			else{
				if(r.nextBoolean()){
		...
		for(i = 1; i < registry.length; i++)
		{
			...
			if(registry[i].y1 < miny) miny = registry[i].y1;
		}
		...
		if(tileCheck(halfw,map.length - (miny + 1))){place(halfw - 1, halfw + 1, miny + 1, max + 1, HALLWAY, false, false);}
		...
		for(i = 0; i < registry.length; i++)
		{
			...
			{
				int j = 0;
				//If the room has been framed go ahead and skip to the next tile
				if(map[map.length - registry[i].y2 - 1][registry[i].x1] == HALLWAY) j++;
	...
	public Ship constructShip(Vector3f loc)
	{
		...
		for(int x = 0; x < map[0].length; x++)for(int y = 0; y < map.length; y++)
		{
			...
			if(tile != SPACE)
			{
				...
				else
				{
					if(roomMap.get(tile) == null)
					{
						...
						//roomMap.put(tile, ship.createRoom(convertLoc(x1,y1)));
		...
		for(int x = 0; x < map[0].length; x++)for(int y = 0; y < map.length; y++)
		{
			...
			if(x == 0 || y == 0)
			{
				if(x == 0)
				{
					if(tile != SPACE)
					{
						if(tile != HALLWAY)ship.addWall(roomMap.get(tile), createWall(x - 1, x, y, y), false);
						else ship.addHallway(createWall(x - 1, x, y, y));
					}
				}
				if(y == 0)
				{
					if(tile != SPACE)
					{
						if(tile != HALLWAY)ship.addWall(roomMap.get(tile), createWall(x, x, y - 1, y), false);
						else ship.addHallway(createWall(x, x, y - 1, y));
					}
				}
				continue;
			}
			...
			if(tile != tile1)
			{
				...
				{
					if(tile1 == HALLWAY)ship.addWall(roomMap.get(tile), createWall(x - 1, x, y, y), true);
					else ship.addWall(roomMap.get(tile), createWall(x - 1, x, y, y), false);
				}
				else if(tile1 != SPACE && tile1 != HALLWAY && tile1 != THRUSTER)
				{
					if(tile == HALLWAY)ship.addWall(roomMap.get(tile1), createWall(x - 1, x, y, y), true);
					else ship.addWall(roomMap.get(tile1), createWall(x - 1, x, y, y), false);
				}
				else
				{
			...
			if(tile != tile2)
			{
				...
				{
					if(tile2 == HALLWAY)ship.addWall(roomMap.get(tile), createWall(x, x, y - 1, y), true);
					else ship.addWall(roomMap.get(tile), createWall(x, x, y - 1, y), false);
				}
				else if(tile2 != SPACE && tile2 != HALLWAY && tile2 != THRUSTER)
				{
					if(tile == HALLWAY)ship.addWall(roomMap.get(tile2), createWall(x, x, y - 1, y), true);
					else ship.addWall(roomMap.get(tile2), createWall(x, x, y - 1, y), false);
				}
				else
				{
		...
		for(int x = 0; x < map[0].length; x++)
		{
			...
			if(tile != SPACE)
			{
				...
				if(tile == HALLWAY) ship.addHallway(wall);
				else ship.addWall(roomMap.get(tile), wall, false);
			}
		}
	...
	public Wall createWall(int x1, int x2, int y1, int y2)
	{
		...
		if((x2 - x1) != 1 && (y2 - y1) != 1) throw new RuntimeException("Tiles aren't adjacent!");
	...
	private class RoomRegistry
	{
		...
		public RoomRegistry(int x1, int x2, int y1, int y2, int id) {
			...
			this.id = id;}
		public int size(){return (int)Math.abs((x1 - x2) * (y1 - y2));}
		public void setRoom(int room){this.room = room;place(x1, x2, y1, y2, room, false, false);}
		public boolean isRoom(int room){return this.room == room;}
		public int frame()
		{
			if(!frame)
			{	
	...
	public static RoomRegistry findRoom(int room, RoomRegistry[] registry)
	{
		...
		{
			if(registry[i].isRoom(room)) return registry[i];
		}
		return null;
	}
	...
	public boolean placeCheck(int x1, int x2, int y1, int y2, int object){return placeCheck(x1,x2,y1,y2,object,true, false);}
	public boolean placeCheck(int x1, int x2, int y1, int y2, int object, boolean xsymmetry, boolean ysymmetry)
	{
		...
		{
			if(xsymmetry && ysymmetry) {if(!tileCheck(map[0].length - x1, map[0].length - x2, map.length - y1, map.length - y2)) return false;}
			else if(xsymmetry) {if(!tileCheck(map[0].length - x1, map[0].length - x2, y1, y2)) return false;}
			else if(ysymmetry) {if(!tileCheck(x1, x2, map.length - y1, map.length - y2)) return false;}
	 ...
	 * @param x1 the starting or ending x coord
	 * @param x2 the starting or ending x coord
	 * @param y1 the starting or ending y coord
	 * @param y2 the starting or ending y coord
	 * @param object the integer value to change to
	 */
	public void place(int x1, int x2, int y1, int y2, int object){place(x1,x2,y1,y2,object,true, false);}
	public void place(int x1, int x2, int y1, int y2, int object, boolean xsymmetry, boolean ysymmetry)
	{
		...
		if(xsymmetry && ysymmetry) {place(map[0].length - x1, map[0].length - x2, map.length - y1, map.length - y2, object, false, false); return;}
		if(xsymmetry) {place(map[0].length - x1, map[0].length - x2, y1, y2, object, false, false); return;}
		if(ysymmetry) {place(x1, x2, map.length - y1, map.length - y2, object, false, false); return;}
	}
	...
	public boolean tileCheck(int x1, int x2, int y1, int y2)
	{
		for(int x = x1; x < x2; x++)for(int y = y1; y < y2; y++)
		{
			if(!tileCheck(x, map.length - y - 1)) return false;
		}
		return true;
	}
	public boolean tileCheck(int x, int y){return map[y][x] == SPACE;}
