...
{	
	...
	public Room(float x, float y, float z){this(new Vector3f(x,y,z), 0, 0, 0);}
	...
	public void generate()
	{
		...
		if(possibleDoors.size() > 0)
		{
			int numDoors = 1;
			int roomsize = x * y;
			if(roomsize > 9 && new Random().nextBoolean()) numDoors++;
			if(numDoors > possibleDoors.size()) numDoors = possibleDoors.size();
			int doorConvert = 0;
	...
	public void addWall(Wall wall){addWall(wall, false);}
	public void addWall(Wall wall, boolean door)
	{
		if(door) wall.attachment = "Door";
		this.walls.add(wall);
	}
