...
public class CollisionDetector 
{
	...
	{
		if(!one.isActive() || !two.isActive()) return 1;
		if(one.isStatic() && two.isStatic()) return 1;
		if(one.body == two.body) return 1;
...
//	}
	...
	public int sphereAndCapsule(Sphere sphere, Capsule capsule, CollisionData data)
	{
		...
		if(!IntersectionTests.sphereAndSphere(sphere, closest, capsule.getRadius())) return 0;
		Contact contact = data.nextContact();
		if(contact== null) return 0;
	...
	public int capsuleAndCapsule(Capsule one, Capsule two, CollisionData data)
	{
		...
		if(!IntersectionTests.sphereAndSphere(closest1, one.getRadius(), closest2, two.getRadius())) return 0;
		Contact contact = data.nextContact();
		if(contact== null) return 0;
	...
	{
		if(one.max.x < two.min.x || one.min.x > two.max.x) return 0;
		if(one.max.y < two.min.y || one.min.y > two.max.y) return 0;
		if(one.max.z < two.min.z || one.min.z > two.max.z) return 0;
