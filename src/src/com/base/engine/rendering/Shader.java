	...
	public void updateUniforms(Transform transform, Material material, RenderingEngine renderingEngine)
	{
		...
		for(int i = 0; i < resource.getUniformNames().size(); i++)
		{
			...
			else if(uniformName.startsWith("T_"))
			{
				...
				else if(uniformName.equals("T_lightMVP"))
					{ 
			...
			else
			{
				...
				else if(uniformType.equals("float"))
				{
					...
					{
						FloatBuffer buffer = (FloatBuffer)material.getBuffer(uniformName);
						if(buffer != null) setUniformfbuffer(uniformName, buffer);
					}
					else setUniformf(uniformName, material.getFloat(uniformName));
				}
				else if(uniformType.equals("int"))
				{
					if(uniformName.contains("["))
					{
						IntBuffer buffer = (IntBuffer)material.getBuffer(uniformName);
						if(buffer != null) setUniformibuffer(uniformName, buffer);
					}
					else setUniformi(uniformName, material.getInt(uniformName));
				}
				else if(uniformType.equals("bool"))
				{
					setUniformi(uniformName, material.getBoolean(uniformName));
				}
				else if(uniformType.equals("mat4"))
				{
					if(uniformName.contains("["))
					{
						if(material instanceof SkeletonMaterial)
						{
							SkeletonMaterial smat = (SkeletonMaterial)material;
							setUniform(uniformName, smat.getBones());
						}
					}
				}
...
//		}
	...
	public void setUniformibuffer(String uniformName, IntBuffer value)
	{
		glUniform1iv(resource.getUniforms().get(uniformName), value);
	}
	
	public void setUniformfbuffer(String uniformName, FloatBuffer value)
	{
		glUniform1fv(resource.getUniforms().get(uniformName), value);
	}
	...
	{
		glUniformMatrix4fv(resource.getUniforms().get(uniformName), true, Util.matricesToFloatBuffer(matrices));
	}
