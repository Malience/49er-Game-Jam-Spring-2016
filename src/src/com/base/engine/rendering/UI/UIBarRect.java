...
public class UIBarRect extends UIElement implements UIRenderable
{
	...
	public UIBarRect(String texturePath, float width, float height){this(texturePath, new Vector2f(width, height));}
	public UIBarRect(String texturePath, Vector2f size){this(texturePath, size, true);}
	...
	public void generate()
	{
		...
		if(!horizontal)
		{
			vertices = new Vector2f[]{
					new Vector2f(position.x + size.x, position.y + size.y * current),
			...
			texCoords = new Vector2f[]{ 
				new Vector2f(1,current),
				new Vector2f(0,current),
				new Vector2f(1,0),
				new Vector2f(0,0)
			};
		}
