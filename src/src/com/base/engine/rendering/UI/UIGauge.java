...
public class UIGauge extends UIElement implements UIRenderable
{
	...
	public UIGauge(String texturePath, float radius, float start, float length){this(texturePath, radius, start, length, 0);}
	...
	{
		Vector2f dir = new Vector3f(0,length,0).rotate(new Vector3f(0,0,1), (float)Math.toRadians(current)).getXY();
		Vector2f position = this.getTransform().getPos().getXY();
		dir = dir.add(position);
		
		pointerVertices = new Vector2f[]{
				new Vector2f(width/2, length).rotate(current),
				new Vector2f(-width/2, length).rotate(current),
		...
		pointerIndices = new int[]{
				0,2,1,
				1,2,3
		};
		
		pointerTexcoords = new Vector2f[]{
			new Vector2f(1,1),
			new Vector2f(0,1),
			new Vector2f(1,0),
			new Vector2f(0,0)
