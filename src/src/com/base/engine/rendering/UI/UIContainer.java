...
public class UIContainer extends UIMenu
{
	...
	public void addSlots(int num)
	{
		...
		if(rows == 1) horizontal = (itemDim + spacing) * slots.size() + spacing;
	...
	public ArrayList<ComponentAttachment> getAllComponentsOf(String ca)
	{
		...
		for(GenericSlot slot : slots)
		{
			...
			if(item != null)
			{
				...
				{
					if(classes[i].getSimpleName().equals(ca)) out.add(item);
				}
			}
		}
		return out;
	...
	public void openAction() 
	{
		...
		if(invpos == null) invpos = new Vector3f(player.inventory.getTransform().getPos());
