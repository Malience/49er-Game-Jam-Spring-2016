...
public class UISlider extends UIObject
{
	...
	//public UISlider(String texturePath, Vector2f size, boolean horizontal){this(texturePath, size, new Vector2f(5,10), 0, horizontal);}
	public UISlider(String texturePath, Vector2f size, Vector2f sliderSize, float start, boolean horizontal) 
	{
		...
		if(horizontal) trackHalfSize = size.x;
		else  trackHalfSize = size.y;
	...
	public Vector3f convert(float pos)
	{
		...
		if(horizontal) return new Vector3f(pos,0,0);
		else return new Vector3f(0,pos,0);
	}
	
	public float convert(Vector3f loc)
	{
		if(horizontal) return loc.x / trackHalfSize + .5f;
		else return loc.y / trackHalfSize + .5f;
	}
	...
	class Slider extends UIRect implements Draggable, UIRenderable
	{
		...
		{	
			...
			if(horizontal)
			{
				...
				if(mousePos.x > center.x + trackHalfSize) mousePos.x = center.x + trackHalfSize;
				else if(mousePos.x < center.x - trackHalfSize)mousePos.x = center.x - trackHalfSize;
			}
			else
			{
				mousePos.x -=  mousePos.x - Input.getPrevMousePosition().x;
				if(mousePos.y > center.y + trackHalfSize) mousePos.y = center.y + trackHalfSize;
				else if(mousePos.y < center.y - trackHalfSize)mousePos.y = center.y - trackHalfSize;
			}
