...
public class UIGaugeBound extends UIGauge implements UIRenderable
{
	...
	float ccwiseBound;
	public UIGaugeBound(String texturePath, float radius, float start, float length){this(texturePath, radius, start, length, 0, -90, 90);}
	public UIGaugeBound(String texturePath, float radius, float start, float length, float pivot, float cwBound, float ccwBound) 
	{
	...
	{
		this.current -= amount;
		if(this.current < clockwiseBound) this.current = clockwiseBound;
	}
	
	public void rotateCounterClockwise(float amount)
	{
		this.current += amount;
		if(this.current > ccwiseBound) this.current = ccwiseBound;
	}
}
