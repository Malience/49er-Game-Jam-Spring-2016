...
public class AnimationComponent extends GameComponent implements Animate
{
	...
	public AnimationComponent(){this(null);}
	public AnimationComponent(Skeleton skeleton){this(skeleton, -1);}
	...
	public void playAnimation(float delta)
	{
		...
		if(transition > 0)
		{
			if(lastPose == null) updateLastPose();
			
			lerpFactor = 1/transition;
			
			for(int i = 0; i < captured.length; i++)
			{
				skeleton.setPose(i, this.interpolate(i, lastPose[i], skeleton.getAnimation(current).getFrame(nextframe).getPose(i), lerpFactor));
			}
			transition -= delta*framerate;
			if(transition < 0) time -= transition;
		}
		else
		{
			for(int i = 0; i < captured.length; i++)
			{
				skeleton.setPose(i, this.interpolate(i, skeleton.getAnimation(current).getFrame(lastframe).getPose(i), skeleton.getAnimation(current).getFrame(nextframe).getPose(i), lerpFactor));
			}
	...
	public void transitionIntoAnimation(int animNum){transitionIntoAnimation(animNum,STANDARD_TRANSITION_TIME);}
	public void transitionIntoAnimation(int animNum, float transitionTime)
	{
		playAnimation(animNum, 0, transitionTime);
	}
	
	public void transitionIntoAnimationAtTime(int animNum, float startTime){transitionIntoAnimationAtTime(animNum,startTime,STANDARD_TRANSITION_TIME);}
	...
	public void transitionIntoNextAnimation(){transitionIntoNextAnimation(STANDARD_TRANSITION_TIME);}
	public void transitionIntoNextAnimation(float transitionTime)
	{
		if(next == -1)
		{
			if(defaultAnim == -1) return;
			transitionIntoAnimation(defaultAnim, transitionTime);
		}
		else transitionIntoAnimation(next, transitionTime);
	}
	...
	{
		if(captured[0][i] && captured[1][i] && captured[2][i]) return skeleton.getTransform(i);
			
		Vector3f pos = null;
		Vector3f scale = null;
		Quaternion rot = null;
		if(!captured[0][i]) pos = one.getPos().lerp(two.getPos(), lerpFactor);
		else pos = skeleton.getTransform(i).getPos();
		if(!captured[1][i])scale = one.getScale().lerp(two.getScale(), lerpFactor);
		else scale = skeleton.getTransform(i).getScale();
		if(!captured[2][i])rot = one.getRot().slerp(two.getRot(), lerpFactor, false);
		else rot = skeleton.getTransform(i).getRot();
	...
	public void setAnimationFramerate(float framerate){this.framerate = framerate;}
	public float getAnimationFramerate(){return framerate;}
	
	public int getCurrentAnimation(){return current;}
	public float getAnimationTime(){return time;}
	public int getNextAnimation(){return next;}
	...
	public void startLooping(){loop = true;}
	public void toggleLooping(){loop = !loop;}
