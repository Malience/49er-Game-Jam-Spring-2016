...
public class text2D {
	...
	public void print(String text, int x, int y, int size) {
		...
		for (int i = 0; i < text.length() * 4; i += 4) {
			...
			vertices[i + 1] = new Vector2f(x + i * size + size, y + size); // Upper
																			// right
			vertices[i + 2] = new Vector2f(x + i * size + size, y); // Bottom
																	// Left
			...
			uvs[i + 1] = new Vector2f(uvx + 1.0f / 16.0f, 1.0f - uvy); // Upper
																		// Right
			uvs[i + 2] = new Vector2f(uvx + 1.0f / 16.0f, 1.0f - (uvy + 1.0f / 16.0f)); // Bottom
																						// Left
			uvs[i + 3] = new Vector2f(uvx, 1.0f - (uvy + 1.0f / 16.0f)); // Bottom
																			// Right
