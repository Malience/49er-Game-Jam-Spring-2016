...
public class IQEModel {
		...
		try {
			...
			while ((line = meshReader.readLine()) != null) {
				...
				if (tokens.length == 0 || tokens[0].equals("#"))
					...
					bones.add(new Bone(tokens[1].substring(1, tokens[1].length() - 2), Integer.parseInt(tokens[2])));
					mode = "joint";
	...
	private Transform parseTransform(String[] tokens) {
		...
		if (tokens[0].equals("pq")) {
			if (length >= 3)
				out.setPos(new Vector3f(Float.valueOf(tokens[1]), Float.valueOf(tokens[2]), Float.valueOf(tokens[3])));
			if (length == 4)
				out.setRot(new Quaternion(0, 0, 0, 1));
			if (length >= 7)
				out.setRot(new Quaternion(Float.valueOf(tokens[4]), Float.valueOf(tokens[5]), Float.valueOf(tokens[6]),
						Float.valueOf(tokens[7])));
			if (length == 8)
				out.setScale(1);
			if (length >= 10)
				out.setScale(
						new Vector3f(Float.valueOf(tokens[8]), Float.valueOf(tokens[9]), Float.valueOf(tokens[10])));
			...
			if (length >= 15)
				out.setScale(
						new Vector3f(Float.valueOf(tokens[13]), Float.valueOf(tokens[14]), Float.valueOf(tokens[15])));
			...
			if (length == 4)
				out.setRot(new Quaternion(0, 0, 0, 1));
			if (length >= 6)
				out.setRot(new Quaternion(0, 0, 0, 1).rotateByVector(
						new Vector3f(Float.valueOf(tokens[4]), Float.valueOf(tokens[5]), Float.valueOf(tokens[6]))));
			if (length == 7)
				out.setScale(1);
			if (length >= 9)
				out.setScale(
						new Vector3f(Float.valueOf(tokens[7]), Float.valueOf(tokens[8]), Float.valueOf(tokens[9])));
		}
		return out;
	}
