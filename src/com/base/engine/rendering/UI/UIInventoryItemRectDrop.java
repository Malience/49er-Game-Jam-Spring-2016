...
public class UIInventoryItemRectDrop extends UIInventoryItem implements UIRenderable, Draggable {
	Vector2f halfSize;

	UIInventoryItemRectDrop(Texture texture, Vector2f halfSize) {
		super(texture);
		this.halfSize = halfSize;
	}

	public UIInventoryItemRectDrop(String texturePath, Vector2f halfSize) {
		super(texturePath);
		this.halfSize = halfSize;
		this.priority = 5;
	}
	...
	public int drop(float delta, Vector2f mousePos, ArrayList<DropLocation> dropLocations) {
		for (DropLocation loc : dropLocations) {
			if (loc instanceof UIDropLocation && loc.drop(this)) {
