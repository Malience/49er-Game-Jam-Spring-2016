...
public class UIMesh {
	...
	// private static HashMap<String, MeshResource> loadedModels = new
	// HashMap<String, MeshResource>();
	 ...
	 * 
	 * @param fileName
	 *            Name of the file containing a mesh. Only accepts .obj files
	 */
	...
	/**
	 * Creates Mesh using vertices and indices. The indices decide the direction
	 * in which an object is drawn
	 * 
	 * @param vertices
	 *            The vertices of the mesh
	 * @param indices
	 *            An array of indices in reference to the vertices array
	 */
	...
	// GL15.glBufferData(GL_ARRAY_BUFFER, Util.createFlippedBuffer(vertices),
	// GL_STATIC_DRAW);
	//
	// glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, resource.getIbo());
	// glBufferData(GL_ELEMENT_ARRAY_BUFFER, Util.createFlippedBuffer(indices),
	// GL_STATIC_DRAW);
	// }
		...
		try {
			...
			while ((line = meshReader.readLine()) != null) {
				...
				if (tokens.length == 0 || tokens[0].equals("#"))
					...
					vertices.add(new Vector2f(Float.valueOf(tokens[1]), Float.valueOf(tokens[2])));
