...
public class IntersectionTests {
	public static boolean AABBandAABB(AABB one, AABB two) {
		...
		return d1x > 0 && d1y > 0 && d1z > 0 && d2x > 0 && d2y > 0 && d2z > 0;
	}
	...
	public static boolean boxAndBox(Box one, Box two) {
		...
		return overlapOnAxis(one, two, (one.getAxis(0)), toCentre)
				&& overlapOnAxis(one, two, (one.getAxis(1)), toCentre)
				&& overlapOnAxis(one, two, (one.getAxis(2)), toCentre) &&

		overlapOnAxis(one, two, (two.getAxis(0)), toCentre) && overlapOnAxis(one, two, (two.getAxis(1)), toCentre)
				&& overlapOnAxis(one, two, (one.getAxis(2)), toCentre) &&
	 ...
	 * 
	 * @param p
	 *            The ray's origin - sphere's center
	 * @param d
	 *            Direction of the ray, normalize it first
	 * @param r
	 *            Radius of the sphere
	 * @param i
	 *            Bucket vector, x contains the first intersection distance and
	 *            y contains the second where 1st <= second
	...
	public static boolean tryAxis(Box one, Box two, Vector3f axis, Vector3f toCentre, int index, Vector2f pen) {
		if (axis.magnitude() * axis.magnitude() < 0.0001)
			return true;
		axis.normalize();
	...
	public static Vector3f contactPoint(Vector3f pone, Vector3f done, float oneSize, Vector3f ptwo, Vector3f dtwo,
			float twoSize, boolean useOne) {
	...
	public static void closestPointLineLine(Vector3f v0, Vector3f v10, Vector3f v2, Vector3f v32, Vector2f out) {
		...
		if (denom != 0)
			out.x = (d0232 * d3210 - d0210 * d3232) / denom;
		else
			out.x = 0;
	...
	// Intersect segment S(t)=sa+t(sb-sa), 0<=t<=1 against cylinder specified by
	// p, q and r
	static boolean intersectSegmentCylinder(Vector3f a, Vector3f b, Vector3f p, Vector3f q, float radius,
			Vector2f out) {
		...
		if (Math.abs(l) < .001f) {
			...
			// Now known that segment intersects cylinder; figure out how it
			// intersects
			if (md < 0.0f)
				out.x = -mn / nn; // Intersect segment against ’p’ endcap
			else if (md > dd)
				out.x = (nd - mn) / nn; // Intersect segment against ’q’ endcap
			else
				out.x = 0.0f; // ’a’ lies inside cylinder
			return true;
		}
		float m = dd * mn - nd * md;
		float discr = m * m - l * c;
