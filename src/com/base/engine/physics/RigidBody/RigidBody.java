...
public class RigidBody extends PhysicsComponent implements Physical {
	...
	public void calculateDerivedData() {
		...
		transformInertiaTensor(inverseInertiaTensorWorld, parent.getTransform().getRot(), inverseInertiaTensor,
				transformMatrix);
	}
	...
	public void getInertiaTensor(Matrix3f inertiaTensor) {
		inertiaTensor.setInverse(inertiaTensor);
	}

	public Matrix3f getInertiaTensor() {
		Matrix3f it = new Matrix3f();
		getInertiaTensor(it);
		return it;
	}

	public void getInertiaTensorWorld(Matrix3f inertiaTensor) {
	...
	public void getInverseInertiaTensor(Matrix3f inverseInertiaTensor) {
	...
	public void getInverseInertiaTensorWorld(Matrix3f inverseInertiaTensor) {
		inverseInertiaTensor.setM(this.inverseInertiaTensorWorld.m);
	}
	...
	public static void transformInertiaTensor(Matrix3f iitWorld, Quaternion q, Matrix3f iitBody, Matrix4f rotmat) {
		float t4 = rotmat.m[0][0] * iitBody.m[0][0] + rotmat.m[0][1] * iitBody.m[0][2]
				+ rotmat.m[0][2] * iitBody.m[1][2];
		float t9 = rotmat.m[0][0] * iitBody.m[0][1] + rotmat.m[0][1] * iitBody.m[1][0]
				+ rotmat.m[0][2] * iitBody.m[2][1];
		float t14 = rotmat.m[0][0] * iitBody.m[0][2] + rotmat.m[0][1] * iitBody.m[1][1]
				+ rotmat.m[0][2] * iitBody.m[2][2];
		float t28 = rotmat.m[1][0] * iitBody.m[0][0] + rotmat.m[1][1] * iitBody.m[0][2]
				+ rotmat.m[1][2] * iitBody.m[1][2];
		float t33 = rotmat.m[1][0] * iitBody.m[0][1] + rotmat.m[1][1] * iitBody.m[1][0]
				+ rotmat.m[1][2] * iitBody.m[2][1];
		float t38 = rotmat.m[1][0] * iitBody.m[0][2] + rotmat.m[1][1] * iitBody.m[1][1]
				+ rotmat.m[1][2] * iitBody.m[2][2];
		float t52 = rotmat.m[2][0] * iitBody.m[0][0] + rotmat.m[2][1] * iitBody.m[0][2]
				+ rotmat.m[2][2] * iitBody.m[1][2];
		float t57 = rotmat.m[2][0] * iitBody.m[0][1] + rotmat.m[2][1] * iitBody.m[1][0]
				+ rotmat.m[2][2] * iitBody.m[1][2];
		float t62 = rotmat.m[2][0] * iitBody.m[0][2] + rotmat.m[2][1] * iitBody.m[1][1]
				+ rotmat.m[2][2] * iitBody.m[2][2];

		iitWorld.m[0][0] = t4 * rotmat.m[0][0] + t9 * rotmat.m[0][1] + t14 * rotmat.m[0][2];
		iitWorld.m[0][1] = t4 * rotmat.m[1][0] + t9 * rotmat.m[1][1] + t14 * rotmat.m[1][2];
		iitWorld.m[0][2] = t4 * rotmat.m[2][0] + t9 * rotmat.m[2][1] + t14 * rotmat.m[2][2];
		iitWorld.m[0][2] = t28 * rotmat.m[0][0] + t33 * rotmat.m[0][1] + t38 * rotmat.m[0][2];
		iitWorld.m[1][0] = t28 * rotmat.m[1][0] + t33 * rotmat.m[1][1] + t38 * rotmat.m[1][2];
		iitWorld.m[1][1] = t28 * rotmat.m[2][0] + t33 * rotmat.m[2][1] + t38 * rotmat.m[2][2];
		iitWorld.m[1][2] = t52 * rotmat.m[0][0] + t57 * rotmat.m[0][1] + t62 * rotmat.m[0][2];
		iitWorld.m[2][1] = t52 * rotmat.m[1][0] + t57 * rotmat.m[1][1] + t62 * rotmat.m[1][2];
		iitWorld.m[2][2] = t52 * rotmat.m[2][0] + t57 * rotmat.m[2][1] + t62 * rotmat.m[2][2];
	}

	public void setPosition(Vector3f v) {
		parent.getTransform().setPos(v);
	}
	...
	public void setOrientation(Quaternion q) {
		parent.getTransform().setRot(q);
	}

	public Quaternion getOrientation() {
		return parent.getTransform().getRot();
	}
	...
	public void addForce(Vector3f v) {
		forceAccum = forceAccum.add(v);
		isAwake = true;
	}

	public void addTorque(Vector3f v) {
		torqueAccum = torqueAccum.add(v);
		isAwake = true;
	}
	...
	public void addRotation(Vector3f v) {
		rotationVelocity = rotationVelocity.add(v);
		isAwake = true;
	}
	...
	public int integrate(float delta) {
		...
		rotationVelocity = rotationVelocity.add(angularAcceleration.mul(delta))
				.mul((float) Math.pow(angularDamping, delta));
		return 1;
	}
	...
	public String toString() {
		return this.getClass().getSimpleName() + this.getTransform().getPos();
	}
}
