...
public class Input {
	...
	public static void init(long window) {
		 ...
		 * 
		 * @Override public void invoke(long window, int key, int scancode, int
		 * action, int mods) { if ( key == GLFW_KEY_ESCAPE && action ==
		 * GLFW_RELEASE ) glfwSetWindowShouldClose(window, GL_TRUE); // We will
		 * detect this in our rendering loop } });
		 */
	...
	public static void drag(float delta) {
		...
		if (dragging != null) {
			if (!dragging.isActive()) {
				dragging = null;
				return;
			} // You shouldn't be dragging that
